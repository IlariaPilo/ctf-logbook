...
assert BYTES, f'{RED}[ Error ]{ENDC} Fill BYTES with the content of exec(marshall.loads(..))'

print(f'{CROSS} unmarshal.py')
print(f'{CROSS} using Python {sys.version.split()[0]}')
print(f'{CROSS} converting byte sequence {BYTES[:6]}...')

# step 1. generate a test pyc with this file
TMP = '__tmp__'
test_code = 'print("Hello, World!")\n'
with open(f'{TMP}.py', 'w') as f:
    f.write(test_code)
py_compile.compile(f'{TMP}.py', cfile=f'{TMP}.pyc')

# step 2. compile same test code using marshall
code_object = compile(test_code, "<string>", "exec")
MARSHAL = marshal.dumps(code_object)[:50]

# step 3. get the intro of the file
with open(f'{TMP}.pyc', 'rb') as f:
    PYC = f.read()

magic_end = PYC.find(MARSHAL)
assert magic_end != -1
MAGIC = PYC[:magic_end]
...
with open('unmarshal.pyc', 'wb') as f:
    # write magic number and other intro bytes
    f.write(MAGIC)
    marshal.dump(code, f)

print(f'{STAR} done! output is in `unmarshal.pyc`, now you just need to decompile it!')
